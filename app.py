...
server = app.server  # WAŻNE dla Render
...
app.layout = html.Div([
    ...
    dcc.Dropdown(
        id="payment-filter",
        options=[{"label": pm, "value": pm} for pm in sorted(df["PaymentMethod"].unique())],
        multi=True
    ),
...
def update_charts(payment_methods):
    ...
    hourly["Weekday"] = pd.Categorical(hourly["Weekday"],
        categories=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"],
        ordered=True)
    fig1 = px.line(hourly, x="Hour", y="Total", color="Weekday", title="Sprzedaż wg godziny i dnia tygodnia")

    # Wykres 2: Heatmapa
    heatmap = dff.pivot_table(index="Weekday", columns="Hour", values="Total", aggfunc="sum", fill_value=0)
    heatmap = heatmap.reindex(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"])
    fig2 = px.imshow(heatmap, labels=dict(x="Godzina", y="Dzień tygodnia", color="Sprzedaż"), title="Heatmapa sprzedaży")
